...
v2.0 引入：替代固定的 TOP_K 截断。引擎根据检索结果的整体质量动态决定保留多少条记忆，以及如何组织它们。
...
现有 258 个单元测试覆盖了各模块的独立功能，但缺少模拟多天对话的行为回归测试。建议从以下场景开始：
1. 用户连续 7 天每天聊 5 轮 → 检查记忆检索质量和人格标签一致性
2. 用户离开 8 小时 → 检查浅巩固是否正确触发
3. 同一事实的情绪翻转 → 检查旧记忆是否被标记 stale
