...
每个节点一个独立测试函数，使用真实组件。
测试环境通过 conftest.py 中的 seeded_env / isolated_env / isolated_env_no_bm 提供。
"""
import json
import math
import os
...
# ═══════════════════════════════════════════════════════════════════
# 辅助工具：从 conftest 拉取种子数据（pytest 运行时 conftest 在 sys.modules 中）
# ═══════════════════════════════════════════════════════════════════

def _get_memory_by_id_safe(ctx, mid):
    """安全版 get_memory_by_id，内置回退实现。"""
    try:
...
def _get_all_memory_ids_safe(ctx):
    """安全版 get_all_memory_ids。"""
    all_mems = ctx.chroma_service.list_all()
